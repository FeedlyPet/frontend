...
async function submit() {
  ...
  try {
    ...
    const err = e as any
    if (err.response?.status === 409) errors.value.email = 'An account with this email already exists'
    else errors.value.general = err.response?.data?.message ?? 'Registration failed'
  } finally {
    loading.value = false
  }
}
...
async function detectTimezone() {
  ...
  try {
    ...
    if (timezones.includes(tz.trim())) timezone.value = tz.trim()
  } catch {
  } finally {
  ...
  <div class="auth-page">
    <div class="auth-card">
      ...
      <form @submit.prevent="submit" class="auth-form">
        <div class="field">
          <label>Name</label>
          <input v-model="name" type="text" required minlength="2" maxlength="100" autocomplete="name" placeholder="Your name" :class="{ error: errors.name }" />
          <p v-if="errors.name" class="field-error">{{ errors.name }}</p>
        </div>

        <div class="field">
          <label>Email</label>
          <input v-model="email" type="email" required autocomplete="email" placeholder="you@example.com" :class="{ error: errors.email }" />
          <p v-if="errors.email" class="field-error">{{ errors.email }}</p>
        </div>

        <div class="field">
          <label>Password</label>
          <input v-model="password" type="password" required minlength="8" autocomplete="new-password" placeholder="••••••••" :class="{ error: errors.password }" />
        ...
        <div class="field">
          ...
          <select v-model="timezone" :disabled="detectingTz">
...
<style scoped>
.tz-label-row { display: flex; align-items: center; justify-content: space-between; }
.tz-detect-btn {
  display: flex; align-items: center; gap: 0.3rem;
  font-size: 0.78rem; font-family: 'Outfit', sans-serif; font-weight: 500;
  background: none; border: none; cursor: pointer; color: #C4875A; transition: color 0.2s; padding: 0;
}
.tz-detect-btn:hover { color: #A0522D; }
.tz-detect-btn.active { color: #A0522D; }
.tz-icon { width: 0.85rem; height: 0.85rem; }
.field select { padding: 0.65rem 0.9rem; border: 1.5px solid #E0D3C0; border-radius: 0.75rem; font-size: 0.95rem; font-family: 'Outfit', sans-serif; color: #2C1208; background: #F5EDE0; outline: none; transition: border-color 0.2s; }
.field select:focus { border-color: #A0522D; background: #fff; }
